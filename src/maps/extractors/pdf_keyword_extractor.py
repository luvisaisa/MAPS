...
class PDFKeywordExtractor(BaseKeywordExtractor):
    ...
    # =========================================================================
    # BaseKeywordExtractor Implementation
    # =========================================================================

    def can_extract(self, file_path: str) -> bool:
        """Check if this extractor can handle PDF files."""
        return file_path.lower().endswith(('.pdf', '.PDF'))

    def extract_keywords(self, file_path: str,
                        extraction_config: Optional[Dict] = None) -> List[ExtractedKeyword]:
        """
        Extract keywords from PDF file (BaseKeywordExtractor interface).

        Args:
            file_path: Path to PDF file
            extraction_config: Optional configuration dict
        ...
        # Extract using PDF-specific method
        metadata, pdf_keywords = self.extract_from_pdf(file_path, store_in_db, max_pages)

        # Convert ExtractedPDFKeyword to ExtractedKeyword
        keywords = []
        for pdf_kw in pdf_keywords:
            keywords.append(ExtractedKeyword(
                text=pdf_kw.text,
                category=pdf_kw.category,
                context=pdf_kw.context,
                source_field=f"page_{pdf_kw.page_number}",
                confidence=1.0,
                frequency=pdf_kw.frequency
            ))
    ...
    def get_supported_categories(self) -> List[str]:
        ...
        return ['metadata', 'abstract', 'keyword', 'body']
